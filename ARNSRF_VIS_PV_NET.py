...
class RNCNNPolicy(ActorCriticPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **kwargs):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            self.relations, self.attention, self.mid_relations = self.attentional_relational_module(self.entities_flatten)  # Note! output shape: [b, n, n, d]
    ...
    def all_step(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            action, value, neglogp, att, mid_relations = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp, self.attention, self.mid_relations],
                                                   {self.obs_ph: obs})
        else:
            action, value, neglogp, att, mid_relations = self.sess.run([self.action, self.value_flat, self.neglogp, self.attention, self.mid_relations],
                                                   {self.obs_ph: obs})
        return action, value, self.initial_state, neglogp, att, mid_relations
    ...
    def attentional_relational_module(self, entities_flattened):
        ...
        return relations_stage_3, att_1, relations_stage_2
...
if __name__ == "__main__":
    ...
    from a2c_visualization_class import A2C
